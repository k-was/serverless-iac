...
// This is a modified version that introduces the necessary changes in order to run on AWS Lambda
// Original: https://github.com/golang/example/tree/master/outyet
// The changes are anotated with go comments and a decription
...
import (
	...
	"github.com/apex/gateway" //Change: This is a drop in replacement for for net/http's ListenAndServe for use in AWS Lambda functions
)
...
func main() {
	...
	// Change: Using the gateway packages ListenAndServe as a replacement for net/https
	// This allows for compatability with lambda with minor code changes.
...
type Server struct {
	...
	wg  sync.WaitGroup // Change: Added a wait group as the polling go routine was not complete  when the lambda was being invoked via API-Gateway
...
func NewServer(version, url string, period time.Duration) *Server {
	...
	s.wg.Add(1) // Change: Adding go routine to wait group.
	go func() {
		defer s.wg.Done()
		s.poll()
	}()
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	s.wg.Wait() // Change: Wait for polling go routine.
	hitCount.Add(1)
	s.mu.RLock()
	data := struct {
		URL     string
	...
	s.mu.RUnlock()
	w.Header().Add("Content-Type", "text/html") //Change: Ensure that the client can view the page as HTML
